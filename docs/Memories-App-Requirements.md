...
| FR-QUOTA-7 | The signed-in user shall see a **Storage Used vs Total indicator in the top bar on every authenticated page** — a labelled bar with used and total, so the figure is visible without navigating to settings. It shall degrade gracefully on narrow screens rather than crowding the bar: **below 640px it moves into the top bar's header menu** (style guide §6) alongside the other actions, where it shows in full. It remains one tap from every page, and never becomes a trip to settings. |
| FR-QUOTA-8 | The indicator shall change appearance as the quota fills, giving a **distinct near-full state** (at ≥ 80%) and a **distinct full state** (at ≥ 100%), so running out is anticipated rather than discovered at upload time. Where the indicator is collapsed behind the header menu (FR-QUOTA-7), **the closed menu button shall carry the warning itself** — a meter nobody has opened cannot anticipate anything. |
