...
- **D20 Comments and likes: OUT of v1.** Descoped by the product owner. Only **people tagging** (FR-SOC-4/5) is built from Section 3.7; FR-SOC-1/2/3 are deferred. The `comments` and `likes` tables remain in the schema, unused — dropping them would need a migration for no benefit, and they are the natural home if the feature returns.
