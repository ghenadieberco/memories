...
- Panel: radius 22px, ~252px wide, anchored under the button's right edge, fades in over .18s. It uses the **denser `--popover` fill (86% white)**, not `.glass`'s 55%, and a deeper `0 18px 50px rgba(108,43,217,.22)` shadow — it floats over page content rather than sitting on the canvas, and at 55% a photo grid reads straight through it. Still the §4 recipe otherwise: blur, white border, purple-tinted shadow.
