...
import { AccessDeniedError, assertCanContribute } from "@/lib/access";
 ...
 *
 * ONE FILE PER REQUEST, by design. The client sends N parallel-ish requests and
 ...
 * working buffers in a 1GB VM, which is how you get an OOM kill halfway through
 * someone's holiday album.
 */
...
export async function POST(request: Request) {
  ...
  const parsed = bodySchema.safeParse({ memoryId: form.get("memoryId") });
  if (!parsed.success) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: `We couldn't store ${file.name}.` },
      { status: 502 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: `We couldn't save ${file.name}.` },
      { status: 500 },
    );
  }
}
