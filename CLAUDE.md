...
- **There is exactly one unauthenticated *write* path: guest upload (D21)**, and it stays that way. It is allowed only via `getPublicMemoryForContribution()`, which additionally requires the owner's per-memory `public_can_contribute` opt-in, and it is rate-limited. Never add a second unauthenticated write, and never widen this one to editing or deleting. It carries **photos and videos alike** (D25 — guest contribution means "add what the app supports"), but **video spends a second, tighter per-IP budget** on top of the general limit; keep that guard if you touch this path. Guest uploads have `uploaded_by = NULL`, so only the memory owner can delete them.
- **Validate every request body with Zod at the server boundary.**
- **Identify uploaded media by sniffing its bytes, never by the client's mime type** — `lib/image.ts` for images, `lib/video.ts` for video. HEIC and MP4 share an ISO-BMFF `ftyp` box, so the brand lists must stay disjoint.
- **Image optimization is mandatory and not user-configurable** (`NFR-OPT`, `FR-PROF-4`): resize to longest edge ~2048 + WebP q~80, generate a ~400px thumbnail, extract EXIF `taken_at` **before** stripping metadata, discard the original (D5). **This applies to images only** — video is stored as uploaded (D23), so for video there is no derived copy and no metadata stripping.
- **Serve images from the Tigris/R2 public URL/CDN** — never proxy image bytes through the app. Downloads obey this too: archives are built in the browser (D24), never streamed from a route.
