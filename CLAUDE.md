...
- **There is exactly one unauthenticated *write* path: guest photo upload (D21)**, and it stays that way. It is allowed only via `getPublicMemoryForContribution()`, which additionally requires the owner's per-memory `public_can_contribute` opt-in, and it is rate-limited. Never add a second unauthenticated write, and never widen this one to editing or deleting — **including to video** (`FR-VIDEO-7`: it is sized for 25 MB photos, not 100 MB anonymous uploads). Guest uploads have `uploaded_by = NULL`, so only the memory owner can delete them.
